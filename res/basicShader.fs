...
void main()
{
    ...
    finalColor = pow(specular*texture2D(specularTexture, texCoord0) + dif * texture2D(texture_diffuse1, texCoord0), vec4(vec3(1.0/1.4), 1.0)); //vec4(vec3(0.5),1.0) //pow(texture2D(diffuseTexture, texCoord0) * dif + specular, vec4(vec3(1.0/1.8), 1.0));// * texture2D(diffuseTexture, texCoord0); //vec2(0.32,0.12));
}